...
const PlaceholderPage: React.FC<{ title: string; route: string; permission: string }> =
  ({ title, route, permission }) => (
    <div>
      <div className="glass-heavy" style={styles.pageHeader}>
        <h1 style={styles.title}>{title}</h1>
        <p style={styles.subtitle}>占位页面 · 接入真实组件</p>
      </div>
      <div className="card-3d" style={{ marginTop: 20 }}>
        <p style={{ color: 'var(--color-text-secondary)' }}>
          路由: <code style={styles.code}>{route}</code> ·
          权限: <code style={styles.code}>{permission}</code>
        </p>
      </div>
    </div>
  );
